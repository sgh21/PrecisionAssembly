...
def is_close(a,b,threshold=5*np.pi/180):
...
def main():
    ...
    # 设置最大速度
    # client.aubo.robot.set_joint_maxacc(joint_maxacc=(3.14, 3.14,3.14,3.14, 3.14, 3.14))
    # client.aubo.robot.set_joint_maxvelc(joint_maxvelc=(3.14, 3.14,3.14,3.14, 3.14, 3.14))
    client.aubo.robot.set_joint_maxacc(joint_maxacc=(2.4, 2.4, 2.4, 2.4, 2.4, 2.4))
    client.aubo.robot.set_joint_maxvelc(joint_maxvelc=(2.4, 2.4, 2.4, 2.4, 2.4, 2.4))
    # client.aubo.robot.set_joint_maxacc(joint_maxacc=(1, 1,1, 1, 1, 1))
    # client.aubo.robot.set_joint_maxvelc(joint_maxvelc=(1, 1,1, 1, 1, 1))
    # 在这里定义三个速度
    a = 0.4
    v_normal = 0.5 # 正常移动
    v_spin = 0.6
    ...
    # print("the ori after modify in camera :",cube1_ori_in_camera[2]*180/.pi)
    cube0_ori_flag = -np.pi/2<= cube0_ori_in_camera[2]<=np.pi/2
    cube1_ori_flag = -np.pi/2<= cube1_ori_in_camera[2]<=np.pi/2
    both_ori_flag = ~(cube0_ori_flag^cube1_ori_flag)
    home_ori_flag = cube0_ori_flag

    delta_theta = ((cube0_ori_in_camera[2]-cube1_ori_in_camera[2])+2*np.pi)% (2*np.pi)
    spin_flag = delta_theta <= np.pi 
    another_dist_01 = 0.005
    another_dist_2 = 0.002
    Z_cube2_ground = 34.7  # 全是理论值，理论上无需调整
    Z_gripper2_ground = 9.5  # 夹爪在末端接触地面时，夹爪中心到地面的高度
    slot_offset = np.array([0.0, 0.0, 0.005])# 和抓取的深度有关，对应不同的抓取位置需要调整 0.0075
    # print(both_ori_flag,home_ori_flag)
    if both_ori_flag and home_ori_flag:
        ...
        another_dist = np.array([0, 0, another_dist_01])
        gripper_init_pos ,gripper_init_ori = client.aubo.get_pose('gripper_center')
        gripper_init_ori = quaternion_to_rpy(np.array(gripper_init_ori))
        cube0_pos_save = cube0_pos_in_camera + safe_dist  # 安全位置c
        cube0_pos_grasp = cube0_pos_in_camera - another_dist # 抓取位置
        waypoints = []
        waypoints.append({'pos':gripper_init_pos,'ori':gripper_init_ori})
        waypoints.append({'pos':cube0_pos_save,'ori':cube0_ori_in_camera})
        waypoints.append({'pos':cube0_pos_grasp,'ori':cube0_ori_in_camera})
        client.aubo.move_trajectory(waypoints,frame_name='gripper_center')
        # cube0_pos_save = cube0_pos_in_camera + safe_dist  # 安全位置c
        ...
        cube1_ori_another = cube1_ori_in_camera.copy() + np.array([0,0,offset]) # 另一侧的安全位置的位姿（直接再多转90，90的正负需要判断）
        client.aubo.robot.set_end_max_line_acc(a)
        client.aubo.robot.set_end_max_line_velc(v_spin) # 0.15
        ...
        if trans_flag2:
            ...
            offset = np.pi/2
        another_dist = np.array([0, 0, another_dist_2]) # 调整物块的抓取高度
        ...
        client.aubo.movel_tf(safe_slot_pos, safe_slot_ori, 'gripper_center',joint=True)
    ...
    elif both_ori_flag and (~home_ori_flag):
        cube0_ori_in_camera[2] = cube0_ori_in_camera[2]-np.pi if cube0_ori_in_camera[2] > np.pi/2 else cube0_ori_in_camera[2]+np.pi
        cube1_ori_in_camera[2] = cube1_ori_in_camera[2]-np.pi if cube1_ori_in_camera[2] > np.pi/2 else cube1_ori_in_camera[2]+np.pi
        trans_flag0 = cube0_ori_in_camera[2] < 0
        trans_flag1 = cube1_ori_in_camera[2] < 0
        trans_flag2 = cube2_ori_in_camera[2] < 0
        # 将相机坐标旋转180°到工件坐标，cube0_ori_in_camera是相机坐标系下的物块实际位姿
    ...
    elif home_ori_flag and spin_flag:
        cube1_ori_another = cube1_ori_in_camera.copy()
        cube1_ori_another[2] = cube1_ori_another[2] + np.pi/2 \
                            if cube1_ori_in_camera[2] < -np.pi/2 \
                            else cube1_ori_another[2] - 3*np.pi/2
        trans_flag0 = cube0_ori_in_camera[2] < 0
        trans_flag1 = cube1_ori_in_camera[2] < 0
        trans_flag2 = cube2_ori_in_camera[2] < 0
        # 将相机坐标旋转180°到工件坐标，cube0_ori_in_camera是相机坐标系下的物块实际位姿
        ...
        cube1_ori_in_camera = rpy_to_quaternion(np.array(cube1_ori_in_camera))
        cube2_ori_in_camera = rpy_to_quaternion(np.array(cube2_ori_in_camera))
        cube1_ori_another = rpy_to_quaternion(np.array(cube1_ori_another))
        cube0_pos_in_camera , cube0_ori_in_camera = client.aubo.tf_tree.transform_pose(cube0_pos_in_camera,cube0_ori_in_camera,'camera_center','world')   # 把原来在A的点转换到B下
        cube1_pos_in_camera , cube1_ori_in_camera = client.aubo.tf_tree.transform_pose(cube1_pos_in_camera,cube1_ori_in_camera,'camera_center','world')   # 把原来在A的点转换到B下
        cube2_pos_in_camera , cube2_ori_in_camera = client.aubo.tf_tree.transform_pose(cube2_pos_in_camera,cube2_ori_in_camera,'camera_center','world')
        _ , cube1_ori_another = client.aubo.tf_tree.transform_pose(cube1_pos_in_camera,cube1_ori_another,'camera_center','world')
        cube0_ori_in_camera = quaternion_to_rpy(np.array(cube0_ori_in_camera))
        cube1_ori_in_camera = quaternion_to_rpy(np.array(cube1_ori_in_camera))
        cube2_ori_in_camera = quaternion_to_rpy(np.array(cube2_ori_in_camera))
        cube1_ori_another = quaternion_to_rpy(np.array(cube1_ori_another))
        ...
        client.aubo.robot.set_end_max_line_velc(v_normal) # 0.15
        client.aubo.movel_relative(safe_dist + another_dist, np.array([0,0,0]), frame_name='gripper_center')  # 夹爪移动到物块1的另一侧抓取位置
        client.gripper.close_gripper(speed=grasp_speed, force=spin_gripper_force) # 夹爪闭合
        time.sleep(0.4)
        client.aubo.movel_relative(np.array([0,0,0]),ori = np.array([0,0,np.pi/2]), frame_name='gripper_center',joint=True)
        client.gripper.open_gripper(speed=grasp_speed) # 夹爪打开
        # client.aubo.movel_relative(- safe_dist - another_dist, np.array([0,0,0]), frame_name='gripper_center')  # 夹爪移动到物块1的另一侧安全位置
        client.aubo.robot.set_end_max_line_acc(a)
        client.aubo.robot.set_end_max_line_velc(v_spin) # 0.15
        # client.aubo.movel_relative(np.array([0,0,0]), np.array([0,0,-offset]), frame_name='gripper_center')  # 夹爪移动到物块1的安全位置（90的正负需要判断）
        client.aubo.movel_tf(cube1_pos_save,cube1_ori_another,frame_name='gripper_center', joint=True)  # 夹爪移动到物块1的安全位置(正常的抓取角度)（90的正负需要判断）
        client.aubo.robot.set_end_max_line_acc(a)
        client.aubo.robot.set_end_max_line_velc(v_normal) # 0.15
        client.aubo.movel_relative(safe_dist + another_dist, np.array([0,0,0]), frame_name='gripper_center')  # 夹爪移动到物块1的抓取位置
        client.gripper.close_gripper(speed=grasp_speed, force=100) # 夹爪闭合
        ...
        client.aubo.movel_tf(init_pos,init_ori,'flange_center')
    elif home_ori_flag and not(spin_flag):
        cube1_ori_another = cube1_ori_in_camera.copy()
        cube1_ori_another[2] = cube1_ori_another[2] - np.pi/2 \
                            if cube1_ori_in_camera[2] > np.pi/2 \
                            else cube1_ori_another[2] + 3*np.pi/2
        danger_ori = cube1_ori_another[2] >= np.pi/2
        if danger_ori:
            cube1_ori_another[2] -= np.pi/2
        trans_flag0 = cube0_ori_in_camera[2] < 0
        trans_flag2 = cube2_ori_in_camera[2] < 0
        # 将相机坐标旋转180°到工件坐标，cube0_ori_in_camera是相机坐标系下的物块实际位姿
        cube0_ori_in_camera[2] += np.pi
        ...
        if not danger_ori:
            ...
            cube0_ori_another = cube0_ori.copy()+np.array([0,0,np.pi/2])
            client.aubo.movel_tf(cube0_pos+safe_dist,cube0_ori_another,frame_name='gripper_center')  # 夹爪移动到物块0的上方
            client.aubo.robot.set_end_max_line_acc(a)
            client.aubo.robot.set_end_max_line_velc(v_insert) # 0.15
            cube0_pos_insert = cube0_pos
            cube0_pos_insert[2] += 0.02  # 这个得调
            client.aubo.movel_tf(cube0_pos_insert, cube0_ori_another, frame_name='gripper_center') # 夹爪移动到装配位
            client.gripper.open_gripper(speed=grasp_speed)  # 夹爪打开
            client.aubo.robot.set_end_max_line_acc(a)
            client.aubo.robot.set_end_max_line_velc(v_normal) # 0.15
            client.aubo.movel_tf(cube0_pos+safe_dist,cube0_ori_another,frame_name='gripper_center')  # 夹爪移动到物块0的上方
        ...
        client.aubo.movel_tf(init_pos,init_ori,'flange_center')
    elif not(home_ori_flag) and spin_flag:
        cube0_ori_in_camera[2] = cube0_ori_in_camera[2]-np.pi if cube0_ori_in_camera[2] > np.pi/2 else cube0_ori_in_camera[2]+np.pi
        cube1_ori_another = cube1_ori_in_camera.copy()
        cube1_ori_another[2] = cube1_ori_another[2] - np.pi/2 
        trans_flag0 = cube0_ori_in_camera[2] < 0
        ...
        client.aubo.movel_tf(init_pos,init_ori,'flange_center')
    elif not(home_ori_flag) and not(spin_flag):
        cube0_ori_in_camera[2] = cube0_ori_in_camera[2]-np.pi if cube0_ori_in_camera[2] > np.pi/2 else cube0_ori_in_camera[2]+np.pi
        cube1_ori_another = cube1_ori_in_camera.copy()
        cube1_ori_another[2] = cube1_ori_another[2] + np.pi/2 
